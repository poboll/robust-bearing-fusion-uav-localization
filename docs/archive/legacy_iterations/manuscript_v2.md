...
This paper addresses that gap with a geometry-preserving robust passive localization framework. The key design idea is not to replace geometric reasoning with a black-box model, but to strengthen it with three complementary mechanisms: consensus-style geometric initialization, trimming-aware residual suppression, and iterative residual reweighting. The resulting framework preserves interpretability while improving stability under bias, outliers, and mixed degradation.
...
The remainder of the paper is organized as follows. Section 2 reviews related work. Section 3 formulates the collaborative bearing-only localization problem. Section 4 presents the proposed method. Section 5 reports regime comparison, ablation, formation generalization, and runtime results. Section 6 discusses implications and limitations. Section 7 concludes the paper.
...
\]
where \(b\) is a common angular bias, \(\epsilon_i\) is nominal noise, and \(o_i\) is occasional outlier corruption. Some observations may be missing.

The objective is to estimate \(p\) from the available bearing set while controlling both nominal error and tail-risk behavior. Because heavy-tailed failures are a central challenge in degraded passive localization, the evaluation is based not only on average error but also on median error, upper-tail error, success rate, and catastrophic failure rate.

## 4. Proposed Method

### 4.1 Consensus Geometric Initialization

Classical bearing-only localization often starts from pairwise line intersections. This is interpretable but fragile. The proposed framework therefore constructs multiple geometric candidates from the intersection cloud:

1. a centroid-style candidate,
2. a median-style candidate,
3. an inlier-focused candidate produced by trimming distant intersections.

The later robust stage selects and refines the best candidate rather than committing to a single brittle initialization.
...
1. residual-based reweighting, which downweights large residuals gradually,
...
This structure prevents a few corrupted measurements from dominating the final estimate.

### 4.3 Optional Bias Correction

The framework also supports a lightweight common-bias correction term through the circular weighted mean of residuals. However, current ablation results show that trimming and reweighting are stronger contributors than bias estimation alone. Accordingly, the main novelty is positioned as robust geometry-preserving refinement rather than as bias estimation in isolation.

### 4.4 Practical Complexity
...
All experiments are synthetic and fully reproducible. The simulator supports:
- common bias,
- missing observations,
- outlier corruption,
- mixed degradation,
- multiple UAV formation layouts.

The baseline family includes least squares, vanilla robust Huber refinement, PSO, simulated annealing, and the proposed robust bias-trimmed method.

### 5.2 Regime Comparison

Figure 1 presents the regime comparison under clean, biased, missing, outlier, and mixed measurements. The proposed method preserves low clean-regime error while showing its strongest advantage in outlier and mixed regimes. In the single-run outlier comparison, the error drops from 4.197 for least squares to 0.120 for the proposed method. In the mixed regime, the error decreases from 9.073 to 0.752. These results support the core claim that the framework is especially useful under degraded bearing corruption rather than only under nominal noise.
...
Figure 2 summarizes the mixed-regime ablation. Removing trimming significantly worsens upper-tail behavior, increasing the mixed-regime \(p90\) from about 1.57 to 3.18. Reducing reweighting also weakens stability. By contrast, removing bias estimation has only a mild effect in the current synthetic setup. This indicates that the real contribution lies in the combination of consensus initialization, trimming, and reweighting rather than in a bias-estimation trick alone.
...
Figure 3 shows median-error behavior across circular, elliptical, perturbed, and random formations. The proposed method consistently improves over least squares in all four cases. The random-formation case is especially strong: the median error decreases from 2.105 with least squares to 0.577 with the proposed method. This result is important because it shows that the framework is not overfit to a single idealized circular formation.
...
Figure 4 compares median runtime. Least squares and vanilla robust Huber are fastest, while the proposed method is more expensive due to candidate evaluation and iterative reweighting. However, its median runtime remains nearly identical to PSO in the current implementation. This is a useful practical result: the proposed method achieves stronger degraded-regime stability without exceeding the runtime of a representative heuristic global search baseline.
...
The proposed framework occupies a useful middle position between fragile classical estimation and opaque heuristic search. It is clearly stronger than least squares under degraded measurements, yet it avoids the instability and interpretability issues associated with relying entirely on stochastic global search. The comparison with PSO is intentionally balanced rather than exaggerated. In the mixed regime, the proposed method and PSO are approximately tied in paired wins, which supports a credible story: the method does not replace all heuristic search in every case, but it provides a more structured and reproducible path to robust localization.

At the same time, several limitations should be acknowledged. First, the current validation is simulation-based. Second, observability-aware scheduling and cooperative recovery are still better treated as future extensions than as fully closed contributions. Third, the current framework does not yet exploit a full collaborative factor-graph formulation. These limitations do not weaken the main contribution, but they should define the scope of the present paper honestly.
...
- Figure 1: `experiments/figure_regime_comparison.png`
- Figure 2: `experiments/figure_ablation_mixed.png`
- Figure 3: `experiments/figure_formation_generalization.png`
- Figure 4: `experiments/figure_runtime_comparison.png`
