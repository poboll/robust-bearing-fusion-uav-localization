...
\Title{Robust Front-End for Corrupted Bearing-Only Fusion in Multi-UAV Passive Localization}
...
\abstract{This paper studies one-shot multi-UAV passive localization when bearing measurements are missing, biased, or grossly corrupted. The focus is the fusion front-end, while observer poses are supplied by an external navigation module and pose uncertainty is evaluated explicitly. The proposed method combines consensus-style geometric seeding, trimmed iteratively reweighted Gauss--Newton refinement, conditional common-bias correction, and an optional adaptive fixed-budget screening stage. In 3000 Monte Carlo cases, the catastrophic-failure rate at a strict $0.2R$ threshold drops from 0.293 to 0.040 in the outlier regime and from 0.273 to 0.068 in the mixed-corruption regime relative to least squares. The method remains statistically competitive with RANSAC while providing the same unified front-end pipeline for outliers, heterogeneous bias, and pose uncertainty. In pseudo-physical dynamic replay, the harsh-regime median error decreases from 0.498 to 0.352. In 1103 PyBullet multi-vehicle replay cycles, median accuracy is broadly tied with strong robust baselines, but the proposed front-end still improves upper-tail tracking error in disturbed replay. The contribution is therefore a reproducible robust front-end for corrupted bearing fusion rather than a complete UAV autonomy stack.}
...
\item a reproducible validation chain comprising 3000 Monte Carlo cases across five corrupted-measurement regimes, a 2285-case fixed-budget screening benchmark, pseudo-physical dynamic replay with delay, jitter, per-sensor bias, and motion-induced disturbance, and a 1103-case PyBullet multi-vehicle replay study.
\end{itemize}
...
\State Compute least-squares, GNC-GM, and RANSAC-style smoother candidates
\If{the least-squares residual pattern is coherent}
...
\If{the refined solution prunes too aggressively or is not clearly better than the smooth candidate}
    \State Return the smooth candidate
\Else
...
To further reduce the gap between static geometry and UAV-like temporal disturbance, we also replay bearing measurements from a multi-vehicle PyBullet environment derived from the \texttt{gym-pybullet-drones} ecosystem \cite{panerati2021learning}. The replay uses circle and ellipse formations with 8 and 10 vehicles, warm-up phases, time-sampled trajectories, gust-like disturbance, delayed observer poses, attitude-induced bearing perturbation, yaw-defined sensor field of view with a payload mounting offset, soft field-of-view dropouts, missing observations, and slowly drifting common bias. The resulting benchmark contains replayed localization cycles over nominal and disturbed regimes. Although the current study still does not reach hardware-in-the-loop, this replay layer follows the broader UAV-validation recommendation that algorithm papers should test against timing and disturbance sources that are closer to actual vehicle dynamics \cite{perezsegui2023bridging}.
...
Figure~\ref{fig:ransaccase} illustrates one representative mixed-corruption case from the benchmark. Several bearings are individually plausible, but a small inconsistent subset is sufficient to pull the consensus solution away from the target. In this window, least squares yields 0.8628 error, RANSAC remains at 0.6197, and the proposed estimator reduces the error to 0.3129 by trimming the most inconsistent bearings before the final refit. The point is specific but useful for multi-error sensing: once residual inconsistency and contamination coexist, a consensus-only front end can still benefit from trimmed reweighting and selective suppression of the most harmful bearings.
...
\caption{PyBullet multi-vehicle replay benchmark used as a stress layer. The proposed front end does not dominate median replay accuracy, but it limits part of the upper tail in disturbed flight-like windows.}
\label{fig:pybullet}
\end{figure}
...
To provide a lightweight downstream utility test, the replayed one-shot cues were also passed to a minimal Kalman filter with chi-square gating. The goal is not to claim a complete tracking contribution, but to test whether a more reliable front-end cue reduces downstream damage in a simple consumer. Table~\ref{tab:trackingproxy} and Figure~\ref{fig:trackingproxy} show that the proposed front-end lowers the overall replay-sequence break rate from 0.7083 for least squares to 0.6667, matching RANSAC overall, while all methods remain tied at 0.8333 under the disturbed replay regime. The main gain in this proxy is therefore not break-rate elimination but tail suppression: the overall 90th-percentile tracking RMSE drops from 28.8386 for least squares and 34.3885 for RANSAC to 16.4994 for the proposed front end, and the disturbed-regime 90th-percentile tracking RMSE drops from 29.8760 and 36.5509 to 13.6756.
...
\caption{Downstream tracking proxy with a minimal Kalman filter. The proposed front end is not a universal break-rate winner, but it substantially reduces upper-tail tracking error in disturbed replay.}
\label{fig:trackingproxy}
\end{figure}

\subsection{Runtime and Reproducibility Implications}

The runtime results show that the robustness gain is not purchased with an obviously impractical computational burden, although the method is clearly slower than simple least-squares fitting. On an Apple M1 Pro CPU with 10 logical cores, macOS 15.7.2, and Python 3.11.11, the proposed front end has a median runtime of 54.00~ms, compared with 26.56~ms for PSO, 5.42~ms for Huber IRLS, 4.97~ms for simulated annealing, and 3.52~ms for least squares. The stage decomposition is also informative: the robust Stage~1 core has median runtime 35.07~ms, while Stage~2 screening adds 2.24~ms and the adaptive gate adds 1.65~ms when invoked. These timings are acceptable for a cycle-level front-end study, but they also make clear that the current implementation has not yet been optimized for hard real-time embedded deployment.
...
This system-level reading also clarifies why the paper still stops short of a full UAV-systems contribution. A real deployment study would need to show how those improved cues affect tracker reacquisition, formation replanning, target confirmation, or operator workload. The present manuscript does not yet close that loop. What it does provide is a reproducible current-cycle front end that can be inserted before those downstream modules and stress-tested under several corrupted-measurement regimes.

\subsection{What Is Still Missing for a Stronger UAV-Systems Claim}

Several limitations remain. The current formulation is planar and centralized. Observer poses are assumed to be supplied by an external navigation module rather than estimated jointly. The paper does not model upstream detection, target association, or sequential tracking. The screening score is still a fixed engineering surrogate rather than a learned or theoretically optimal budgeted selector, even though the perturbation study shows that its behavior is not tied to one numerically brittle coefficient vector. Finally, the validation chain now includes pseudo-physical and PyBullet replay, but still stops short of hardware-in-the-loop or field data. These limitations do not invalidate the algorithmic contribution, but they do define its current publication scope and the work still needed for a harder UAV-systems target.

\subsection{Scope Boundaries for the Current Manuscript}

The contribution should therefore be read as a robust sensing front end rather than as a complete UAV-systems paper. It is strongest when the question is whether the current sensing cycle can still produce a usable cue after a subset of bearings becomes corrupted. Stronger system-level claims would require SITL, HITL, field replay, or onboard integration studies in addition to the present simulation-based evidence.

\subsection{Practical Research Path from the Current Draft}

The results also suggest a clear next-step research path rather than ending at a static algorithm benchmark. The first step is to preserve the present front-end interface and connect it to a replayed or simulated downstream tracker with richer state and target-motion assumptions. The second step is to replace the current generic bearing corruption model with payload-specific perturbations, for example camera field-of-view truncation, heading misalignment tied to attitude estimates, or radio direction-finding uncertainty tied to signal strength. The third step is to move from centralized fusion toward a bandwidth-aware swarm implementation in which the same adaptive screening logic and consensus initialization are evaluated under explicit uplink timing and V2V communication constraints. These extensions would keep the narrative coherent because they all build on the same front-end question: whether the current sensing cycle yields a cue that downstream modules can still trust.

This staged path is useful for interpreting the contribution fairly. The present paper does not need to solve every systems layer at once to have application value. What it provides is a robust and reproducible front-end baseline around which those stronger system experiments can be organized. In that sense, the contribution is not only an algorithmic result but also a controlled experimental scaffold: it identifies the corruption regimes that matter, fixes a baseline set of strong and weak competitors, and exposes where additional realism should be injected next.

\section{Conclusions}

This paper formulates the contribution as a corrupted-bearing localization problem and presents a robust front end for that problem. The consensus-assisted estimator suppresses catastrophic one-shot failures caused by bad bearings, while the optional adaptive screening module improves over simpler fixed-budget subset policies without replacing all-sensor fusion when the full measurement set is available. Across 3000 Monte Carlo cases and 720 pseudo-physical dynamic replay cases, the method consistently improves over least squares and remains competitive with strong robust baselines such as RANSAC. In 1103 PyBullet replay cycles, the same method should be interpreted more narrowly as a stress-tested front end: replay median accuracy is broadly comparable to the strongest baselines, while tail-oriented tracking error still improves in the disturbed regime.

The main value of the study is therefore methodological and reproducibility-oriented: it provides a robust front-end benchmark for passive bearing fusion under outliers, mixed corruption, observer-pose uncertainty, and heterogeneous bias. Future work should add hardware-coupled validation, extend the formulation to full three-dimensional localization, connect the front-end output to sequential tracking or downstream task-level utility metrics, and study distributed variants that move parts of the consensus initialization or screening logic onto the UAV network itself.
